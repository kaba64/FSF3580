...
```
"""
function restart_arnoldi(A,k,p,tol,v1,number_elements)
    # Step (1) initialize with a trivial Arnoldi factorization
    v1=v1/norm(v1);
    V=v1;
    H=v1'*A*v1;
    r=A*v1-v1*H;
    # ******* k : the number of largest Ritaz values *******************
    RitazVal = zeros(Complex,k,number_elements);
    # Step (2)  (Initiation phase of Arnoldi factorization)
    # *********** Compute an Arnoldi factorization of length 1+(k-1), given
    # a factorization of length 1. **************************
    ...
    for m=1:100   # Outer loop (i.e. restarts)
        # Step (3).(1)
        print("iteration: ",m,"\n")
        # ************ If r zero, the Ritaz paris are the eigenparis. It can
        # be used as a criteria for the breakdown ***********************
        normr=norm(r)

        if (norm(r)<tol)
            restart_count=m-1
            display("finished");
            return V,H,r,restart_count,RitazVal
        end
        ...
        H_Eigenvals = eigvals(H);
        RitazVal[:,m] = H_Eigenvals[1:k];
        VQ=V*Q;
